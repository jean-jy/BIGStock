...
export default function App() {
  ...
  useEffect(() => {
    // onAuthStateChange fires INITIAL_SESSION on mount (covers getSession),
    // then SIGNED_IN / SIGNED_OUT / TOKEN_REFRESHED for future changes.
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      // Show UI immediately with fallback/session data, then enrich with profile
      ...
      // Fetch profile in background and update user if richer data is available
      supabase
        .from('profiles')
        .select('*')
        ...
        .catch(() => {
          // profiles table may not exist or RLS may block — that's fine
        });
    });
